...
public class ShadowLayout extends RelativeLayout {
     ...
     * 底部无阴影时是否默认有padding值
     */
    private boolean mHasBottomPadding = true;
    /**
     * 顶部是否默认有多余的5dp padding的值
     */
    private boolean mHasTopPadding = true;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        float effect = mShadowRadius + dip2px(4);
        float rectLeft = 0;
        float rectTop = 0;
        ...
        if ((mShadowSide & TOP) == TOP) {
            if (mHasTopPadding) {
                ...
                rectTop = mShadowRadius;
                paddingTop = (int) mShadowRadius;
            }
        }
        ...
        if ((mShadowSide & BOTTOM) == BOTTOM) {
            if (mHasBottomPadding) {
                rectBottom = this.getMeasuredHeight() - effect;
                paddingBottom = (int) effect;
            } else {
                rectBottom = this.getMeasuredHeight() - mShadowRadius;
                paddingBottom = (int) mShadowRadius;
            }
        }
    ...
    public void setShadowSide(int side) {
        mShadowSide = side;
    }

    public void setHasBottomPadding(boolean hasBottomPadding) {
        mHasBottomPadding = hasBottomPadding;
    }

    public void setHasTopPadding(boolean hasTopPadding) {
        mHasTopPadding = hasTopPadding;
    }

    public void setOnlyPaddingTop(int onlyPaddingTop) {
        this.onlyPaddingTop = onlyPaddingTop;
    }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (typedArray != null) {
            ...
            mHasBottomPadding = typedArray.getBoolean(R.styleable.ShadowLayout_hasBottomPadding, true);
            mHasTopPadding = typedArray.getBoolean(R.styleable.ShadowLayout_hasTopPadding, true);
            typedArray.recycle();
        }
        setUpShadowPaint();
    }
